...
type DeepLinkRefreshCardResult = { status: "ready" | "not_found" | "rate_limited" | "pending" | "error"; message: string };
...
// 2026-09-21: "이 상품 왜 생겼지?"를 관리자가 저를 거치지 않고 화면에서 바로 판단할 수 있도록,
// 상품이 어느 경로로 등록됐는지(source)를 사람이 읽을 수 있는 한글 라벨로 보여준다.
// - search: 방문자가 가신 사이트에서 검색 후 상세 페이지를 열거나 찜했을 때 등록
// - collection: 수집기(확장 프로그램)가 실제 페이지를 방문해 처음 관측했을 때 등록
// - goldbox / bestcategory: 공식 파트너스 API의 골드박스·베스트카테고리 정기 갱신이 등록
const PRODUCT_SOURCE_LABELS: Record<string, string> = {
  search: "검색 (방문자 상세보기·찜)",
  collection: "수집기 자동 관측",
  goldbox: "골드박스 정기 갱신",
  bestcategory: "베스트카테고리 정기 갱신",
};
const describeProductSource = (source: string) => PRODUCT_SOURCE_LABELS[source] ?? source;
const PRODUCT_RETENTION_DAYS = 90;
const describeDaysUntilAutoDelete = (deactivatedAt: Date | null) => {
  if (!deactivatedAt) return null;
  const elapsedDays = Math.floor((Date.now() - new Date(deactivatedAt).getTime()) / (24 * 60 * 60 * 1000));
  const remainingDays = PRODUCT_RETENTION_DAYS - elapsedDays;
  return remainingDays > 0 ? `자동 삭제까지 D-${remainingDays}` : "자동 삭제 대기 중(다음 정리 작업에서 삭제)";
};
...
export default function AdminPrices() {
  ...
  // 2026-09-22: "이관되면 기존내용은 숨김 처리" — isActive: false(SKU 이관·관리자 병합·
  // 골드박스 탈락)인 상품은 기본적으로 이 목록에서 숨긴다. 삭제는 90일 뒤 서버 정리
  // 작업(runRetentionDailySchedule)이 자동으로 하므로, 여기서는 보이지 않게만 한다.
  ...
  const [isBulkDeletingCurrentPrice, setIsBulkDeletingCurrentPrice] = useState(false);
  ...
  const deleteAllMissingSearches = trpc.adminPrices.deleteAllMissingSearches.useMutation({
    onSuccess: async result => {
      setMissingSearchPage(1);
      await utils.adminPrices.listMissingSearches.invalidate();
      toast.success(`검색 실패 이력 ${result.deletedCount}건을 모두 삭제했습니다.`);
    },
    onError: error => toast.error(error.message),
  });
  ...
  // 대량 삭제는 서버 배치 한도(CURRENT_PRICE_DELETE_BATCH_SIZE)를 넘으면 deleteSelectedCurrentProducts가
  // 여러 번 나눠 호출한다 — 성공 토스트·목록 갱신·선택 해제는 그 함수에서 전체가 끝난 뒤 한 번만 처리한다
  // (여기서 onSuccess로 처리하면 배치마다 토스트가 중복으로 뜸).
  const deleteCurrentPriceProducts = trpc.adminPrices.deleteCurrentPriceProducts.useMutation({ onError: error => toast.error(error.message) });
  ...
  const nameBasedOptionBackfill = trpc.adminPrices.backfillMissingOptionMetadataFromNames.useMutation({
    onSuccess: async result => {
      await Promise.all([utils.adminPrices.listMissingOptions.invalidate(), utils.catalog.list.invalidate(), utils.catalog.homeFeatured.invalidate()]);
      toast.success(result.detail);
    },
    onError: error => toast.error(error.message),
  });
  ...
  const currentPriceFiltered = useMemo(() => {
    const visibleScope = showHiddenProducts ? (currentPriceProducts.data ?? []) : (currentPriceProducts.data ?? []).filter(product => product.isActive !== false);
    const filtered = filterAdminProducts(visibleScope, currentPriceQuery);
    return showOnlyMissingOption
      ...
      : filtered;
  }, [currentPriceProducts.data, currentPriceQuery, showOnlyMissingOption, showHiddenProducts]);
  ...
  const hiddenProductCount = useMemo(() => (currentPriceProducts.data ?? []).filter(product => product.isActive === false).length, [currentPriceProducts.data]);
  ...
  // 페이지(10개)뿐 아니라 검색 조건에 맞는 전체 결과(수천~수만 개일 수 있음)를 한 번에 선택할 수 있어야
  // "옵션 미확인 7,908개"처럼 페이지를 넘나드는 대량 정리가 가능하다.
  const currentPriceFilteredIds = useMemo(() => currentPriceFiltered.map(product => product.id), [currentPriceFiltered]);
  const allCurrentFilteredSelected = currentPriceFilteredIds.length > 0 && currentPriceFilteredIds.every(productId => selectedCurrentPriceIds.has(productId));
  ...
  const deleteAllMissingSearchesItems = () => {
    const total = missingSearches.data?.length ?? 0;
    if (total === 0) return;
    if (!window.confirm(`검색 실패 이력 전체 ${total}건을 모두 삭제할까요?\n삭제 후에는 같은 검색어가 다시 실패할 때 새 이력으로 집계됩니다.`)) return;
    deleteAllMissingSearches.mutate();
  };
  ...
  // 서버 라우터(server/routers.ts)의 deleteCurrentPriceProducts 입력 스키마와 맞춰야 한다 — 이 값을
  // 바꾸면 그쪽 z.array(...).max(...)도 같이 바꿔야 100개 넘는 선택이 거부되지 않는다.
  const CURRENT_PRICE_DELETE_BATCH_SIZE = 500;

  const deleteSelectedCurrentProducts = async () => {
    const productIds = Array.from(selectedCurrentPriceIds);
    if (productIds.length === 0) return;
    if (!window.confirm(`선택한 ${productIds.length.toLocaleString("ko-KR")}개 가격 추이 상품을 영구 삭제할까요?\n가격 이력과 찜·알림 연결도 함께 삭제되며 되돌릴 수 없습니다.`)) return;

    setIsBulkDeletingCurrentPrice(true);
    let deletedCount = 0;
    let skippedCount = 0;
    try {
      for (let start = 0; start < productIds.length; start += CURRENT_PRICE_DELETE_BATCH_SIZE) {
        const batch = productIds.slice(start, start + CURRENT_PRICE_DELETE_BATCH_SIZE);
        setBulkDeleteProgress({ done: start, total: productIds.length });
        try {
          const result = await deleteCurrentPriceProducts.mutateAsync({ productIds: batch });
          deletedCount += result.deletedCount;
          skippedCount += result.skippedCount;
        } catch {
          // 개별 배치 실패는 mutation의 onError가 이미 토스트로 알렸다 — 남은 배치는 중단한다.
          break;
        }
      }
    } finally {
      setBulkDeleteProgress(null);
      setIsBulkDeletingCurrentPrice(false);
    }

    setSelectedCurrentPriceIds(new Set());
    await Promise.all([utils.adminPrices.listCurrentPriceProducts.invalidate(), utils.catalog.list.invalidate(), utils.favorites.list.invalidate()]);
    if (deletedCount > 0) toast.success(`${deletedCount.toLocaleString("ko-KR")}개 상품을 삭제했습니다.${skippedCount ? ` ${skippedCount.toLocaleString("ko-KR")}개는 이미 삭제되어 건너뛰었습니다.` : ""}`);
  };
  ...
  return <section>
    ...
    <section className="mb-4 rounded-3xl bg-[#176b3a] p-4 text-white shadow-[0_12px_30px_rgba(23,107,58,.2)]"><div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-2xl bg-white/15"><RefreshCw className="size-4" /></span><div className="min-w-0 flex-1"><p className="text-xs font-extrabold">가신 수집 데이터 동기화</p><p className="mt-1 text-[10px] leading-4 text-[#d9f0dd]">수집기가 보낸 최신 가격을 옵션 SKU가 일치하는 상품에 반영합니다. 외부 수집 이력은 90일 후 자동 정리됩니다.</p><p className="mt-1 text-[10px] leading-4 text-[#d9f0dd]">옵션이 비어 있는 상품은 아래 버튼으로 최근 수집 관측의 용량·수량·포장 정보를 한 번에 보완할 수 있습니다.</p><p className="mt-1.5 text-[10px] font-bold text-[#d9f0dd]">{collectorSyncStatus.data?.finishedAt ? `최근 동기화 ${formatDate(collectorSyncStatus.data.finishedAt)} · ${collectorSyncStatus.data.detail ?? `${collectorSyncStatus.data.processedCount}개 반영`}` : "아직 수집 데이터 동기화 기록이 없습니다."}</p><p className="mt-1 text-[10px] leading-4 text-[#d9f0dd]">수집기 관측이 아예 없는 옵션 미확인 상품은 아래 버튼으로 상품명에서 용량·수량을 다시 파싱해봅니다. 이름에 규격 정보가 없는 상품은 여전히 미확인으로 남습니다.</p></div></div><div className="mt-3 grid gap-2 sm:grid-cols-3"><button type="button" onClick={() => collectorSync.mutate()} disabled={collectorSync.isPending || collectorMetadataSync.isPending || nameBasedOptionBackfill.isPending} className="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-xl bg-white px-4 text-xs font-extrabold text-[#176b3a] disabled:opacity-60"><RefreshCw className={`size-3.5 ${collectorSync.isPending ? "animate-spin" : ""}`} />{collectorSync.isPending ? "수집 데이터 반영 중" : "가격·수집 데이터 동기화"}</button><button type="button" onClick={() => collectorMetadataSync.mutate()} disabled={collectorSync.isPending || collectorMetadataSync.isPending || nameBasedOptionBackfill.isPending} className="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-xl border border-white/60 bg-white/15 px-4 text-xs font-extrabold text-white disabled:opacity-60"><RefreshCw className={`size-3.5 ${collectorMetadataSync.isPending ? "animate-spin" : ""}`} />{collectorMetadataSync.isPending ? "옵션 보완 중" : "옵션 메타 일괄 보완"}</button><button type="button" onClick={() => nameBasedOptionBackfill.mutate()} disabled={collectorSync.isPending || collectorMetadataSync.isPending || nameBasedOptionBackfill.isPending} className="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-xl border border-white/60 bg-white/15 px-4 text-xs font-extrabold text-white disabled:opacity-60"><RefreshCw className={`size-3.5 ${nameBasedOptionBackfill.isPending ? "animate-spin" : ""}`} />{nameBasedOptionBackfill.isPending ? "이름 분석 중" : "이름 기반 옵션 일괄 보완"}</button></div></section>
    <section className="mb-6 rounded-3xl bg-white p-4 shadow-sm ring-1 ring-[#e2ebe1]"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold text-[#308154]">CURRENT PRICE TREND</p><h2 className="mt-1 text-lg font-extrabold">현재 가격 추이 상품</h2><p className="mt-1 text-[11px] leading-4 text-[#718071]">상품명·쿠팡 SKU·옵션으로 검색하고, 확인 가격 입력·품절·삭제·쿠팡 바로가기를 관리합니다. 가격 갱신이 오래된 상품부터 표시하며 1페이지에 10개씩 표시합니다. SKU 이관·관리자 병합·골드박스 탈락으로 비활성화된 상품은 기본적으로 숨겨지며, 90일 뒤 자동 삭제됩니다 — 위 "숨김 상품" 버튼으로 확인할 수 있습니다.</p></div><button type="button" onClick={() => currentPriceProducts.refetch()} className="grid size-10 shrink-0 place-items-center rounded-full bg-[#f4fbf5] text-[#176b3a]" aria-label="현재 가격 추이 새로고침"><RefreshCw className={`size-4 ${currentPriceProducts.isFetching ? "animate-spin" : ""}`} /></button></div><div className="mt-3 flex items-center gap-2 rounded-xl border border-[#d9e6d9] bg-[#fbfdfb] px-3"><Search className="size-4 shrink-0 text-[#6d8975]" /><input value={currentPriceQuery} onChange={event => setCurrentPriceQuery(event.target.value)} placeholder="상품명, 쿠팡 SKU, 옵션·용량·수량 검색" className="min-w-0 flex-1 bg-transparent py-3 text-xs outline-none placeholder:text-[#9aaa9d]" />{currentPriceQuery ? <button type="button" onClick={() => setCurrentPriceQuery("")} className="grid size-7 place-items-center rounded-full text-[#6d8975] hover:bg-[#eaf6ec]" aria-label="현재 가격 추이 검색어 지우기"><X className="size-3.5" /></button> : null}</div><div className="mt-2 flex flex-wrap items-center gap-2"><button type="button" aria-pressed={showOnlyMissingOption} onClick={() => { setShowOnlyMissingOption(previous => !previous); setCurrentPricePage(1); setSelectedCurrentPriceIds(new Set()); }} className={`min-h-9 rounded-xl px-3 text-[10px] font-bold ring-1 ${showOnlyMissingOption ? "bg-[#94601a] text-white ring-[#94601a]" : "bg-[#fffaf0] text-[#94601a] ring-[#ecd09f]"}`}>{showOnlyMissingOption ? "옵션 미확인 전체 보기" : `옵션 미확인 ${missingOptionCount}개`}</button>{showOnlyMissingOption ? <span className="text-[10px] text-[#94601a]">옵션·용량·수량·포장 정보가 모두 없는 상품만 표시합니다.</span> : null}<button type="button" aria-pressed={showHiddenProducts} onClick={() => { setShowHiddenProducts(previous => !previous); setCurrentPricePage(1); setSelectedCurrentPriceIds(new Set()); }} className={`min-h-9 rounded-xl px-3 text-[10px] font-bold ring-1 ${showHiddenProducts ? "bg-[#646b66] text-white ring-[#646b66]" : "bg-[#f0f2f0] text-[#646b66] ring-[#dbe0da]"}`}>{showHiddenProducts ? "숨김 상품 다시 숨기기" : `숨김(이관·비활성) 상품 ${hiddenProductCount}개`}</button>{showHiddenProducts ? <span className="text-[10px] text-[#646b66]">SKU 이관·관리자 병합·골드박스 탈락으로 비활성화된 상품입니다. 비활성화 후 90일이 지나면 자동 삭제됩니다.</span> : null}</div><div className="mt-2 flex flex-wrap items-center justify-between gap-2 rounded-xl bg-[#f4fbf5] px-3 py-2"><div className="flex flex-wrap items-center gap-3"><label className="inline-flex min-h-8 cursor-pointer items-center gap-2 text-[10px] font-bold text-[#45634d]"><input type="checkbox" checked={allCurrentPricePageSelected} onChange={() => setSelectedCurrentPriceIds(previous => { const next = new Set(previous); if (allCurrentPricePageSelected) currentPricePageIds.forEach(productId => next.delete(productId)); else currentPricePageIds.forEach(productId => next.add(productId)); return next; })} className="size-4 accent-[#176b3a]" />현재 페이지 전체 선택</label><label className="inline-flex min-h-8 cursor-pointer items-center gap-2 text-[10px] font-bold text-[#45634d]"><input type="checkbox" checked={allCurrentFilteredSelected} onChange={() => setSelectedCurrentPriceIds(allCurrentFilteredSelected ? new Set() : new Set(currentPriceFilteredIds))} className="size-4 accent-[#176b3a]" />검색 결과 전체 선택 ({currentPriceFilteredIds.length.toLocaleString("ko-KR")}개)</label></div><span className="text-[10px] text-[#718071]">검색 결과 {currentPriceFiltered.length.toLocaleString("ko-KR")}개 · 선택 {selectedCurrentPriceIds.size.toLocaleString("ko-KR")}개</span>{selectedCurrentPriceIds.size > 0 ? <button type="button" onClick={deleteSelectedCurrentProducts} disabled={isBulkDeletingCurrentPrice} className="min-h-8 rounded-xl bg-[#a44c45] px-3 text-[10px] font-bold text-white disabled:opacity-50">{isBulkDeletingCurrentPrice ? (bulkDeleteProgress ? `삭제 중 (${bulkDeleteProgress.done.toLocaleString("ko-KR")}/${bulkDeleteProgress.total.toLocaleString("ko-KR")})` : "삭제 중") : `선택 상품 ${selectedCurrentPriceIds.size.toLocaleString("ko-KR")}개 삭제`}</button> : null}</div>{currentPriceProducts.isLoading ? <p className="py-8 text-center text-xs text-[#718071]">가격 추이 상품을 불러오는 중입니다.</p> : null}{!currentPriceProducts.isLoading && currentPriceFiltered.length === 0 ? <p className="mt-3 rounded-2xl bg-[#f4f7f3] p-4 text-center text-xs text-[#718071]">검색 조건에 맞는 가격 추이 상품이 없습니다.</p> : null}<div className="mt-3 space-y-3">{currentPricePageItems.map(product => { const [coupangProductId, storedItemId, storedVendorItemId] = product.externalProductId.split(":"); const coupangUrl = product.deepLinkUrl ?? null; const coupangDirectUrl = coupangProductId ? createCoupangProductUrl(coupangProductId, storedItemId, storedVendorItemId) : null; const displayName = getProductDisplayName(product.name, product.variantLabel, product.unitLabel); const currentOptionDraft = optionDrafts[product.id] ?? createOptionDraft(product.variantLabel, product.unitLabel); const isEditingCurrentOption = editingOptionIds.has(product.id); return <article key={product.id} className="rounded-2xl border border-[#e3ece4] bg-[#fbfdfb] p-3"><label className="mb-2 inline-flex min-h-8 cursor-pointer items-center gap-2 text-[10px] font-bold text-[#45634d]"><input type="checkbox" checked={selectedCurrentPriceIds.has(product.id)} onChange={() => setSelectedCurrentPriceIds(previous => { const next = new Set(previous); if (next.has(product.id)) next.delete(product.id); else next.add(product.id); return next; })} className="size-4 accent-[#176b3a]" />이 상품 선택</label><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><h3 className="text-xs font-extrabold text-[#25362a]">{displayName}</h3><p className="mt-1 text-[10px] text-[#607765]">현재 옵션·용량·수량: <strong>{product.variantLabel || "미등록"}</strong>{product.quantity ? ` · 수량 ${product.quantity}개` : ""}</p></div><button type="button" onClick={() => { setEditingOptionIds(previous => { const next = new Set(previous); next.add(product.id); return next; }); setOptionDrafts(previous => ({ ...previous, [product.id]: currentOptionDraft })); }} className="min-h-8 shrink-0 rounded-xl border border-[#b9d8c0] bg-white px-2.5 text-[10px] font-bold text-[#176b3a]">{isEditingCurrentOption ? "수정 중" : "옵션 수정"}</button></div><p className="mt-1 text-[10px] text-[#718071]">현재가 {won(product.currentPrice)} · 최저가 {won(product.lowestPrice)}</p><p className="mt-1 text-[10px] text-[#829184]">마지막 확인 {formatDate(product.lastSeenAt)} · SKU {product.externalProductId}</p><p className="mt-1 text-[10px] text-[#829184]">출처 {describeProductSource(product.source)} · 등록 {formatDate(product.firstSeenAt)}</p>{product.isActive === false && product.deactivatedAt ? <p className="mt-1 text-[10px] font-bold text-[#646b66]">{describeDaysUntilAutoDelete(product.deactivatedAt)}</p> : null}{product.lastRefreshReason ? <p className="mt-1 rounded-lg bg-[#fdeceb] px-2 py-1.5 text-[10px] leading-4 text-[#a44c45]">{product.lastRefreshReason}</p> : null}</div>{product.inStock === false ? <span className="shrink-0 rounded-full bg-[#f0f2f0] px-2 py-1 text-[10px] font-bold text-[#646b66]">품절</span> : null}{product.isActive === false ? <span className="shrink-0 rounded-full bg-[#fdeceb] px-2 py-1 text-[10px] font-bold text-[#a44c45]">비활성(만료 감지)</span> : null}</div>{isEditingCurrentOption ? <div className="mt-3 rounded-2xl bg-[#fffaf0] p-3"><p className="text-[10px] font-bold text-[#94601a]">현재 옵션·용량·수량 수정</p><p className="mt-1 text-[10px] leading-4 text-[#876f44]">가격과 SKU는 변경하지 않고 현재 구성 정보만 수정합니다.</p><div className="mt-2 grid gap-2 sm:grid-cols-3"><input value={currentOptionDraft.option} onChange={event => updateDraft(product.id, "option", event.target.value)} placeholder="옵션명" className="rounded-xl border border-[#eadfbd] bg-white px-3 py-2.5 text-xs outline-none focus:border-[#a36c12]" /><input value={currentOptionDraft.capacity} onChange={event => updateDraft(product.id, "capacity", event.target.value)} placeholder="용량 예: 80ml" className="rounded-xl border border-[#eadfbd] bg-white px-3 py-2.5 text-xs outline-none focus:border-[#a36c12]" /><input inputMode="numeric" value={currentOptionDraft.quantity} onChange={event => updateDraft(product.id, "quantity", event.target.value.replace(/[^0-9]/g, ""))} placeholder="수량 예: 2" className="rounded-xl border border-[#eadfbd] bg-white px-3 py-2.5 text-xs outline-none focus:border-[#a36c12]" /></div><button type="button" onClick={() => saveCombinedProduct(product.id)} disabled={updateOptions.isPending} className="mt-2 min-h-9 w-full rounded-xl bg-[#176b3a] px-3 text-[10px] font-bold text-white disabled:opacity-50">{updateOptions.isPending ? "저장 중" : "옵션 정보 저장"}</button></div> : null}<div className="mt-3 flex flex-wrap gap-2"><input inputMode="numeric" value={drafts[product.id] ?? ""} onChange={event => setDrafts(previous => ({ ...previous, [product.id]: event.target.value }))} placeholder="확인 가격(원)" className="min-h-9 min-w-0 flex-1 rounded-xl border border-[#d9e6d9] bg-white px-3 text-xs outline-none focus:border-[#6d9ec7]" /><button type="button" onClick={() => savePrice(product.id)} disabled={save.isPending} className="min-h-9 rounded-xl bg-[#176b3a] px-3 text-[10px] font-bold text-white disabled:opacity-50">저장</button>{coupangUrl ? <a href={coupangUrl} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-9 items-center gap-1 rounded-xl border border-[#b9d8c0] bg-white px-3 text-[10px] font-bold text-[#176b3a]"><ExternalLink className="size-3" />딥링크 바로가기</a> : <span className="inline-flex min-h-9 items-center gap-1 rounded-xl border border-[#dbe0da] bg-[#f4f6f4] px-3 text-[10px] font-bold text-[#8a968b]" title="파트너스 딥링크가 아직 없습니다. 옆의 SKU 직접 열기로 확인한 뒤 수집되면 생성됩니다.">딥링크 대기</span>}{coupangDirectUrl ? <a href={coupangDirectUrl} target="_blank" rel="noopener noreferrer" title="SKU로 만든 쿠팡 직접 주소입니다(관리자 확인용). 로그아웃 상태에서 여세요." className="inline-flex min-h-9 items-center gap-1 rounded-xl border border-[#cbdcf2] bg-[#f6f9ff] px-3 text-[10px] font-bold text-[#3567a8]"><ExternalLink className="size-3" />SKU 직접 열기</a> : null}<button type="button" onClick={() => markCurrentProductSoldOut(product.id, product.name)} disabled={markCurrentPriceProductSoldOut.isPending || product.inStock === false || product.isActive === false} className="min-h-9 rounded-xl border border-[#ecd09f] bg-[#fffaf0] px-3 text-[10px] font-bold text-[#94601a] disabled:opacity-50">품절</button><button type="button" onClick={() => deleteCurrentProduct(product.id, product.name)} disabled={deleteCurrentPriceProduct.isPending} className="min-h-9 rounded-xl border border-[#edc8c3] bg-[#fff8f7] px-3 text-[10px] font-bold text-[#a44c45] disabled:opacity-50">삭제</button></div></article>; })}</div><PageControls page={currentPricePage} total={currentPriceFiltered.length} onChange={setCurrentPricePage} label="현재 가격 추이" /></section>
    ...
    {deferredPageItems.map(product => {
      ...
      const link = needsPurchaseFallback ? null : product.deepLinkUrl ?? null; // 2026-09-22: 딥링크로만 쿠팡 접속(원본 경로 대체 없음)
      ...
      // 2026-09-25: 관리자 화면의 "쿠팡 상품 바로가기"는 SKU로 만든 직접 주소를 다시 쓴다 —
      // 딥링크가 없는 상품(정확 SKU 미확인)은 관리자가 직접 들어가 옵션을 확인해야만
      // 살릴 수 있는데, 딥링크만 허용하면 그 복구 경로 자체가 없어지기 때문이다.
      // 고객 구매 버튼과 수집기 자동 순회는 여전히 딥링크만 쓴다.
