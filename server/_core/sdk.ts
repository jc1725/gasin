...
class SDKServer {
  ...
  async getSuspensionStatus(req: Request) {
    ...
    if (!session || session.openId.startsWith(CRON_OPEN_ID_PREFIX)) return null;
    const user = await db.getUserByOpenId(session.openId);
    if (!user?.isSuspended) return null;
    return {
      isSuspended: true as const,
      reason: user.suspensionReason || "운영 정책에 따라 이용이 일시 정지되었습니다.",
